...
const login = async (c) => {
    ...
    const roles = await authRespository.getUserRoles(foundUser.id);
    const payload = { id: Number(foundUser.id), email: foundUser.email, roles, exp: Math.floor(Date.now() / 1000) + 60 };
